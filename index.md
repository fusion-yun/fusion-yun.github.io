...
FuYun 分两层。**Sp 平台层**是与领域无关的底座：数据内核（统一命名空间下的多后端数据访问）、
计算与建模内核（本体的规范原生执行引擎）、治理—控制内核（受控写入、审计、沙箱、配额、
致动联锁）。**Fy 聚变域层**建于其上：托卡马克集成建模与分析、平衡与输运物理求解、
装置描述数据，以及端到端复现与跨码对拍的基准工作流。

当前对外发布的是其中两项：

| 发布物 | 说明 |
| :--- | :--- |
| **SpO / FyO 本体** | 上层本体与聚变域本体的命名空间及编译产物（见上）|
| **fylite** | 自足的平衡求解与重构内核（零外部依赖）；本站演示由其编译为 WebAssembly，以二进制形式分发 |
